...
在本教程余下部分中你将会向 Department 实体添加一个 ``rowversion`` 的跟踪属性，创建一个控制器和视图，并测试验证一下是否工作正常。
...
如果你倾向于使用 fluent API，可以使用 ``IsConcurrencyToken`` 方法来指定跟踪属性，如下例所示：
...
这一改变将会把标题改为「Departments」，重新排列字段，并使用 Administrator 列代替 RowVersion 列。
...
在 HttpGet 的 ``Edit`` 方法和 ``Details`` 方法中，预加载 ``Administrator`` 导航属性。 
...
你可以通过使用无跟踪查询（no-tracking query）来获取当前数据库的值。
...
这段代码为受影响的系执行查询。因为你已经检查了删除，在这里使用 ``SingleAsync`` 方法比使用 ``SingleOrDefaultAsync`` 方法更好。
...
接下来，代码为每一个在页面（用户编辑过的 Edit 页面）上的值与数据库中的值不同的列添加一个定制错误信息（为简洁起见，此处仅显示一个字段）。
...
用下列代码替换 HttpPost ``Delete`` 方法（名为 ``DeleteCondirmed``）中的代码：
...
如果系已被删除，``AnyAsync`` 方法会返回 false，然后应用程序返回到 Index 方法。
...
至此完成了对处理并发异常的介绍。更多有关 EF Core 中如何处理异常的资料可以阅读 `并发冲突 <https://ef.readthedocs.io/en/latest/saving/concurrency.html>`__。下一篇教程将介绍如何为 Stdent 和 Instructor 实体实现表层次结构继承。
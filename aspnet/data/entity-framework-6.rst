...
作者：`Paweł Grudzień`_ 、 `Damien Pontifex`_、`Tom Dykstra`_
...
要使用 Entity Framework 6，你的项目需要基于完整 .NET Framework 编译，因为 Entity Framework 6 不支持 .NET Core。如果你需要跨平台功能，你需要升级到 `Entity Framework Core`_。
...
在ASP.NET Core 1.0 应用中使用 Entity Framework 6 推荐的方式是将 EF6 上下文和模型类放在一个类库项目（ *.csproj* 项目文件）作用于整个框架。在 ASP.NET Core 项目添加对类库的引用。参考示例 `包含 ASP.NET Core 项目与 EF6 的解决方案 <https://github.com/aspnet/Docs/tree/master/aspnet/data/entity-framework-6/sample/>`__.
...
向  ASP.NET Core 1.0 项目添加 EF6 上下文是不可行的，因为基于 *.xproj* 的项目不支持所有的 EF6 命令功能，如 `Enable-Migrations` 。在未来的 ASP.NET Core 版本，Core 项目将基于 *.csproj* 文件，到那时，你将可直接添加 EF6 上下文到 ASP.NET Core 项目。
...
添加 EF6 上下文而不区分项目类型的话，则只有 EF6 命令行工具支持 EF6 上下文。例如，``Scaffold-DbContext`` 只在 Entity Framework Core 中有效。如果你需要数据库逆向引擎将数据库转为 EF6 模型，参见 `已有数据库情况下的 Code-First <https://msdn.microsoft.com/en-us/jj200620>`__.
...
你的  ASP.NET Core  项目需要引用完整的 .NET framework 和 EF6 。例如，*project.json* 将看起来类似于下面的示例（只展示了相关的文件部分）。
...
因为你的 EF6 上下文没有无参数构造函数，你的 EF6 项目需要提供 `IDbContextFactory <https://msdn.microsoft.com/library/hh506876>`__ 的实现。EF6 命令行工具可以发现并使用这一实现，然后实例化上下文。这里有个例子。
...
至于可运行的示例应用，参见 `简单的 Visual Studio 解决方案 <https://github.com/aspnet/Docs/tree/master/aspnet/data/entity-framework-6/sample/>`__ ，它与本文相互映衬。

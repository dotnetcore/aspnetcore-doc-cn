...
在 ASP.NET Core 中，有多种途径对应用程序状态进行管理，取决于状态是何时以及如何接收到的。本文简要介绍几种方法，并着重介绍为 ASP.NET Core 应用程序安装和配置会话状态支持。
...
`查看或下载示例代码<https://github.com/aspnet/Docs/tree/master/aspnet/fundamentals/app-state/sample>`_
...
`应用程序状态`指的是用于表示应用程序当前表现的任何数据。包括全局的和用户特定的数据。之前版本的ASP.NET（甚至ASP）都自带了对全局的``Application``和``State``状态储存的支持，以及其他很多选项的支持。
...
.. note:: 注意：``Application``储存和ASP.NET的``Cache``特性几乎一样，只是少了一些功能。在 ASP.NET Core 中，``Application`` 已经没有了；可以用 Caching 来实现代替 Application 的功能，从而把针对之前的ASP.NET版本所写的应用程序升级到ASP.NET Core。
...
当数据仅用于一个请求之中时，用``Items``集合储存是最好的方式。数据将在每个请求结束之后被丢弃。可以作为组件和中间件之间,在一个请求期间的不同时间点进行通讯的最佳手段。
...
在查询字符串中添加数值、或利用POST发送数据，可以将一个请求的状态数据提供给另一个请求。这种技术不应该用于敏感数据，因为这些技术手段需要将数据发送到客户端，然后再发送回服务器。这种方法也最好用于少量的数据。查询字符串对以持久的方式获取状态特别有用，可以将状态嵌入链接通过电子邮件或社交网络发送，以备日后使用。然而，用户提交的请求是无法预期的，由于带有查询字符串的URL很容易被分享，必须小心以避免 `跨站请求伪装（CSRF）`<https://www.owasp.org/index.php/Cross-Site_Request_Forgery_(CSRF)>攻击。(例如，即时假设只有通过验证的用户才可以访问基于查询字符串的网址执行请求，攻击者还是可能会诱骗已经验证过的用户去访问这样的网址)。
...
与状态有关的非常小量的数据可以储存在 Cookies 中。他们会随每次请求被发送，所以应该保持最小的尺寸。理想情况下，应该只使用一个标识符，而真正的数据储存在服务器端某处，键值对应着这个标识符。
...
会话储存依靠一个基于 Cookie 的标识符来访问与给定浏览器（来自一个特定机器的特定浏览器的一系列访问请求）会话相关的数据。你不能假设一个会话限定了一个用户，因此要慎重考虑在会话中储存哪些信息。这是用来储存应用程序状态的好地方，针对特定会话，但又不要求永久保持（或者说，需要的时候可以再从持久储存中重新获取）。
...
缓存提供了一种储存和快速访问基于开发者定义键的任意数据的手段。它提供了一套基于时间和其他因素来使缓存项目过期的规则。详情请阅读 :doc:`/performance/caching/index'。
...
配置（Configuration）可以被认为是应用程序状态储存的另外一种形式，不过通常它在程序运行过程中是只读的。详情请阅读 :doc:`configuration`。
...
``HttpContext`` 抽象提供了一个简单的 ``IDictionary<object, object>`` 类型的字典集合，叫做 ``Items``。在每个请求中，这个集合从一个 ``HttpRequest`` 的开始起就可以使用，直到请求结束后被丢弃。要存取集合，你可以简单地给键控项赋值，或根据给定键查询值。

For example, some simple :doc:``middleware`` could add something to the ``Items`` collection:

举个例子，一些简单的 :doc:``中间件`` 可以在 ``Items`` 集合中增加一些内容：
...
.. note::  ``Items`` 的键是简单的字符串，所以如果你在开发需要跨越多个应用程序工作的中间件，你可能要用一个唯一标识符给键名增加前缀以避免键名冲突。（如：采用"MyComponent.isVerified"，而非简单的"isVerified"）。
...
ASP.NET Core 发布了一个会话程序包，里面提供了用于管理会话状态的中间件。你可以通过在 project.json 文件中加入引用来安装这个程序包：
...
程序包一旦安装好，Session 必须在你的应用的 ``Startup`` 类中进行配置。Session 是建立在 ``IDistributedCache`` 之上的，因此你必须把它配置好，否则会得到一个错误。
...
ASP.NET 提供了 ``IDistributedCache`` 的多种实现，包括一个 in-memory 选项（仅用于开发期间和测试）。要采用 in-memory 配置会话，请将以下代码添加到 ``ConfigureServices``：
.. code-block:: c#
...
.. note:: 如果你在 ``UseSession`` 调用之前尝试访问 ``Session`` ，你会得到一个 ``InvalidOperationException`` 异常，说“Session 还没有在这个应用或请求中配置好。”
...
.. warning:: 如果在你已经开始向 ``Response`` 响应流中写入内容之后再尝试创建一个新的 ``Session`` （比如，还没有创建会话 cookie），你将会得到一个 ``InvalidOperationException`` 异常，说“不能在开始响应之后再建立会话。”
...
这些默认值，和默认的 ``IdleTimeout`` （独立于 cookie 在服务端使用），都可以在通过 ``SessionOptions`` 配置 ``Session`` 的时候覆盖重写，如下所示：
...
``IdleTimeout`` 在服务端用于决定在会话被抛弃之前可以闲置多久。任何来到网站的请求穿过 Session 中间件（无论这中间件对 Session 是读取还是写入）都会重置会话的超时时间。
...
.. note:: ``Session`` 是*无锁*的，因此如果两个请求都尝试修改会话的内容，最后一个会成功。此外，``Session`` 被实现为一个*连续会话*，就是说所有的内容都是一起储存的。这就意味着，如果两个请求是在修改会话中不同的部分（不同的键），他们还是会互相造成影响。
...
因为 ``Session`` 是建立在 ``IDistributedCache`` 之上的，你总是需要把要储存的对象实例序列化。因此，这个接口使用 ``byte[]`` 而不是简单的 ``object``。不过，有扩展方法可以让我们在使用诸如 ``String`` 和 ``Int32`` 的简单类型时更加容易。
...
如果要储存更复杂的对象，你需要把对象序列化为一个 ``byte[]`` 字节流以便储存，而后在获取对象的时候，还要从 ``byte[]`` 字节流进行反序列化。
...
相关的示例程序演示了如何使用 Session ，包括储存和获取简单类型以及自定义类型。为了便于观察会话过期后会发生什么，示例中将会话的超时时间配置为短短的10秒：
...
这个默认的行为是由下面这些 *Startup.cs* 里的中间件产生的，当尚未建立会话的请求到来的时候，这些中间件就会执行（注意高亮部分）：
...
``GetOrCreateEntries`` 是一个辅助方法，它会从 ``Session`` 获取一个 ``RequestEntryCollection`` 集合，如果没有则创建一个空的，然后返回得到的集合。这个集合保存一些 ``RequestEntry`` 对象实例，用来跟踪当前会话期间的用户发出的不同请求，以及他们对每个路径发出了多少请求。
...
.. note:: 要储存在会话中的类型必须用 ``[Serializable]`` 标记为可序列化的。
...
如果对象实体存在于 ``Session`` ，则会以 ``byte[]`` 字节流的类型获取到，然后利用 ``MemoryStream`` 和 ``BinaryFormatter`` 将它反序列化，如上所示。如果``Session``中没有这个对象，这个方法则返回一个新的 ``RequestEntryCollection`` 实例。
...
刷新页面会使计数增加；再刷新几次之后，回到网站的根路径，显示如下结果，概括了会话期间发起的所有请求：
...
建立会话是由一个中间件通过处理访问"/session"的请求来完成的。
...
访问这个路径的请求会获取或创建一个 ``RequestEntryCollection`` 集合，再把当前路径加到里面，最后用辅助方法 ``SaveEntries`` 把集合储存到会话中去，如下所示：
...
这个示例中还有一段中间件的代码值得注意，就是映射 "/untracked" 路径。可以在下面看看它的配置：
...
注意这个中间件是在调用``app.UseSession``被调用（第13行）**之前**就配置好的。因此，``Session``的功能对这个中间件来说还不能用，那么访问到这个中间件的请求是不会重置会话的 ``IdleTimeout``。为了证实这一点，你可以在 /untracked 页面上反复刷新10秒钟，再回到首页查看。你会发现它显示你的会话已经超时了，即使你最后一次刷新到现在根本没有超过10秒钟。

...
会话（ ``Session`` ）储存依靠一个基于 Cookie 的标识符来访问与给定浏览器（来自一个特定机器和特定浏览器的一系列访问请求）会话相关的数据。你不能假设一个会话只限定给了一个用户，因此要慎重考虑在会话中储存哪些信息。这是用来储存那种针对具体会话，但又不要求永久保持的（或者说，需要的时候可以再从持久储存中重新获取的）应用程序状态的好地方。
...
举个例子，一个简单的 :doc:``中间件`` 可以在 ``Items`` 集合中增加一些内容：
...
因为 ``Session`` 是建立在 ``IDistributedCache`` 之上的，所以总是需要序列化被储存的对象实例。因此，这个接口使用 ``byte[]`` 而不是简单的 ``object``。不过，有扩展方法可以让我们在使用诸如 ``String`` 和 ``Int32`` 的简单类型时更加容易。

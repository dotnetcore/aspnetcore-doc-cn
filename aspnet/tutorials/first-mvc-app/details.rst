...
代码先行（Code First）模式使用 SingleOrDefaultAsync 方法更易于数据搜索。这个方法包含的一个重要的安全功能，就是在代码尝试用电影记录做任何操作之前确保查找方法已经找到了一条电影记录。例如，黑客可以把产生的链接 URL 从 *http://localhost:xxxx/Movies/Details/1*  改成类似于 *http://localhost:xxxx/Movies/Details/12345* （或者其他非实际电影记录的值），从而给网站带来错误。如果你不检查影片是否为空，应用程序将会抛出异常。

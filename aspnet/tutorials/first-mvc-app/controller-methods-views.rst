...
:doc:`Form Tag Helper </mvc/views/working-with-forms>` 生成一个隐藏域的防伪标记必须和 Movies controller 的 ``Edit`` 方法的 ``[ValidateAntiForgeryToken]`` 产生的防伪标记相匹配。更多信息请参考 :doc:`../../security/anti-request-forgery` 。

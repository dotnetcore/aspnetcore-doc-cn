...
在这一节里，你将添加一些类来管理数据库里的电影数据。这些类是指 **M**\VC 应用程序中的 "**M**\odel"。
...
你将使用一个被称为 `Entity Framework Core <http://ef.readthedocs.org/>`__ 的 .NET 框架数据访问技术来定义和使用这些数据模型类。Entity Framework Core（通常也被称为 **EF** Core）有一个特色的开发模式被称为 *Code First*。你先写代码，然后数据库表会根据这些代码创建。Code First 允许你可以通过编写简单的类来创建数据模型对象（也称为 POCO 类，全称是“plain-old CLR objects”）。数据库将根据这些类创建。如果你需要先创建数据库，你仍然可以按照本教程来学习 MVC 和 EF 应用程序开发。
...
在解决方案资源管理器中，右键点击 *Models* 文件夹 > **添加** > **类** 。类命名为 **Movie** 并添加以下属性：
...
- MoviesController (*Controllers/MoviesController.cs*)
...
Visual Studio 为你自动的创建 `CRUD <https://en.wikipedia.org/wiki/Create,_read,_update_and_delete>`__ （创建、读取、更新、以及删除）Action 方法和视图（自动的创建 CRUD 操作方法和视图被称为 *搭建基架*）。很快你将拥有一个可以让你创建、查看、编辑以及删除电影条目的完整功能的 Web 应用程序。
...
这是一个重要的的错误消息，我们按照这些指令让数据库为我们的电影应用程序做好准备。
...
.. note:: 你或许不能在 ``Price`` 字段输入小数点或逗号。为了使 `jQuery 验证 <http://jqueryvalidation.org/>`__ 支持非英语环境用逗号（","）表示小数点，以及非美式英语日期格式支持 `jQuery 验证 <http://jqueryvalidation.org/>`__ ，你必须采取措施使你的应用程序国际化。查看 `扩展资源`_ 获取更多信息。现在，只能输入整数，比如 10。
...
点击 **Create** 提交表单到服务器，将电影数据保存到数据库中。然后你被重定向到 `/Movies`，你可以在列表中看到新创建的电影。

.. image:: adding-model/_static/h.png

Create a couple more movie entries. Try the **Edit**, **Details**, and **Delete** links, which are all functional.

再创建几个电影条目。尝试 **Edit** 、 **Details** 和 **Delete** 链接（这些也是目前的所有功能）。
...
构造函数使用 :doc:`依赖注入  </fundamentals/dependency-injection>` 将数据库上下文注入到控制器。数据库上下文在控制器的每个 `增查改删（CRUD） <https://en.wikipedia.org/wiki/Create,_read,_update_and_delete>`__ 方法中被使用。
...
对 MoviesController 的请求将返回 ``Movies`` 表中所有的条目，然后传递数据到 ``Index`` 视图（View）。
...
在上文你已了解控制器如何使用 ``ViewData`` 字典传递数据或对象到视图模板 (View Template)。``ViewData`` 字典是一个动态的对象，它提供一个方便后期绑定的方式将信息传递到视图（View） 。
...
MVC 还具有将强类型对象传递到视图模板（View Template）的能力。这种强类型的方式可以更好的在编译时检查你的代码，并在 Visual Studio 中具有丰富的 `IntelliSense <https://msdn.microsoft.com/en-us/library/hcw1s69b.aspx>`__ 体验。当 ``MoviesController`` 创建了方法（Action）和视图（View）时，VS 会使用这种基架机制（即通过强类型模型）。
...
- Controller 对应 ``movies`` （URL 第一个部分）
- Action 对应 ``details`` （URL 第二个部分）
- id 对应 1 （URL 最后一部分）
...
如果能发现到电影，``Movie`` 模型（Model）的实例将被传递到 ``Details`` 视图（View）。
...
检查 *Views/Movies/Details.cshtml* 文件中的内容：
...
通过在视图模板（View Template）文件顶部引入一个 ``@model`` 语句，你可以指定视图（View）所期望的对象类型。当你创建这个 MoviesController 时，Visual Studio 自动在 *Details.cshtml* 文件顶部引入下面的 ``@model`` 语句。
...
这个 ``@model`` 指令允许你使用一个强类型 ``Model`` 对象访问控制器（Controller）传递到视图（View）的电影对象。例如，在 *Details.cshtml* 模板中，代码通过 ``DisplayNameFor`` 和 ``DisplayFor`` HTML 帮助类（HTML Helper）用强类型 ``Model`` 对象传递每个电影字段。 ``Create`` 和 ``Edit`` 方法及其视图模板（View Template）也传递了 ``Movie`` 模型（Model）对象。
...
检查 *Index.cshtml* 视图模板 (View Template) 和 MoviesController 里的 ``Index`` 方法。注意观察代码在调用 View 方法时是如何创建一个 ``List`` 对象的。这段代码将 ``Movies`` 列表从 ``Index`` 操作方法传递给视图（View）：
...
当你创建这个 MoviesController 时，Visual Studio 自动的在 *Index.cshtml* 文件顶部引入如下 ``@model`` 语句：
...
因为 ``Model`` 对象是强类型的（为 ``IEnumerable<Movie>`` 对象），在循环中的每一项的类型都是 ``Movie``。除了其它好处外，这意味着你可以在编译时检查代码并且在代码编辑器里得到完整的 `IntelliSense <https://msdn.microsoft.com/en-us/library/hcw1s69b.aspx>`__ 支持：

.. image:: adding-model/_static/ints.png

You now have a database and pages to display, edit, update and delete data. In the next tutorial, we'll work with the database.

现在你有一个数据库和用于显示、编辑、更新以及删除数据的页面。在下一个教程中，我们将学习利用数据库进行开发工作。

...
ASP.NET Core MVC 支持局部试图，这在你需要重用在不同视图间共享的网页的局部的时候是非常有用的。
...
局部视图是在其它的视图中被渲染的视图。输出的HTML在执行局部视图时被生成，被渲染到调用它的视图（或父视图）。跟视图一样，局部视图也使用 *.cshtml* 文件扩展名。
...
.. note:: 如果你转变自 ASP.NET Web 窗体背景，局部视图与 `用户控件 <https://msdn.microsoft.com/en-us/library/y6wb1a0e.aspx>`_ 类似。

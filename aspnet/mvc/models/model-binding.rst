...
MVC 中的模型绑定从 HTTP 请求参数中将数据映射到 Action 方法里。这些参数可以是 string， interger， float 这样的简单类型，也可以是复杂类型数据。MVC 通过抽象绑定去解决了这些问题，所以开发者们不必继续在每一个app中反复编写同样的代码.自己编写文本到类型的转换代码是冗长并且容易出错的。
